...
TEST(TypeReflectionTest, GetDimensions) {
  ...
  auto test_get_dimensions = [](Decl *D, const std::vector<InterOp::TCppIndex_t> &truth_dims)
  {
    std::vector<InterOp::TCppIndex_t> dims = InterOp::GetDimensions(InterOp::GetVariableType(D));
