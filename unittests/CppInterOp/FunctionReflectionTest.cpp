...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_ConstructorInGetClassMethods) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    struct S {
      int a;
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_GetFunctionReturnType) {
  ...
  EXPECT_EQ(Cpp::GetTypeAsString(Cpp::GetFunctionReturnType(Cpp::GetNamed(
                "func", Cpp::InstantiateTemplate(Decls[15], args2.data(), 1, false)))),
            "double");
}
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_InstantiateTemplateFunctionFromString) {
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_InstantiateFunctionTemplate) {
  std::vector<Decl*> Decls;
  std::string code = R"(
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_GetClassTemplatedMethods_VariadicsAndOthers) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    class MyClass {
      public:
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_InstantiateVariadicFunction) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    class MyClass {};
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch0) {
  ...
  fn = Cpp::InstantiateTemplate(Decls[0], explicit_args1.data(),
                                explicit_args1.size(), false);
  EXPECT_EQ(fn, fn0);

  fn = Cpp::InstantiateTemplate(Decls[0], explicit_args2.data(),
                                explicit_args2.size(), false);
  EXPECT_EQ(fn, fn0);
}

TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch1) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    class MyTemplatedMethodClass {
      public:
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch2) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    template<typename T>
    struct A { T value; };
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch3) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    template<typename T>
    struct A {
  ...
  Cpp::GetOperator(
      ...
      Cpp::Operator::OP_Minus, candidates, Cpp::OperatorArity::kBoth);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch4) {
  std::vector<Decl*> Decls, SubDecls;
  std::string code = R"(
    template<typename T>
    struct A { T value; };
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_BestOverloadFunctionMatch5) {
  std::vector<Decl*> Decls;
  std::string code = R"(
    template<typename T1, typename T2>
  ...
  Cpp::TCppScope_t callme = Cpp::InstantiateTemplate(
      Decls[0], explicit_params.data(), explicit_params.size(), false);
  EXPECT_TRUE(callme);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_GetFunctionAddress) {
  ...
  Cpp::TCppScope_t add1_double =
      Cpp::InstantiateTemplate(funcs[0], argument.data(), argument.size(), false);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_JitCallAdvanced) {
  ...
  EXPECT_TRUE(Cpp::Destruct(object, Decls[1], true, 0));
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_JitCallDebug) {
  ...
  void* result = Cpp::Allocate(Decls[0], 1);
  EXPECT_DEATH(
  ...
  // Destructors
  Cpp::TCppScope_t scope_C = Cpp::GetNamed("C", 0);
  Cpp::TCppObject_t object_C = Cpp::Construct(scope_C, nullptr, 1);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_GetFunctionCallWrapper) {
  ...
  Cpp::GetOperator(TOperator, Cpp::Operator::OP_Less, operators, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(operators.size(), 1);

  Cpp::TCppScope_t op_templated = operators[0];
  auto TAI = Cpp::TemplateArgInfo(Cpp::GetType("int"));
  Cpp::TCppScope_t op = Cpp::InstantiateTemplate(op_templated, &TAI, 1, false);
  ...
  Cpp::GetOperator(Cpp::GetScope("N2", Cpp::GetScope("N1", 0)), Cpp::Operator::OP_Plus,
                   operators, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(operators.size(), 1);
  ...
  Cpp::TCppScope_t consume = Cpp::BestOverloadFunctionMatch(
      unresolved_candidate_methods, {},
      {Cpp::GetVariableType(Cpp::GetNamed("consumable", 0))});
  EXPECT_TRUE(consume);
  ...
  Cpp::TCppScope_t KlassProduct = Cpp::GetNamed("KlassProduct", 0);
  EXPECT_TRUE(KlassProduct);

  Cpp::TCppScope_t KlassProduct_int =
      Cpp::InstantiateTemplate(KlassProduct, &TAI, 1, false);
  ...
  operators.clear();
  Cpp::GetOperator(KlassProduct_int, Cpp::Operator::OP_Star, operators, Cpp::OperatorArity::kBoth);
  ...
  Cpp::TCppScope_t TemplatedEnum = Cpp::GetScope("TemplatedEnum", 0);
  EXPECT_TRUE(TemplatedEnum);

  auto TAI_enum =
      Cpp::TemplateArgInfo(Cpp::GetTypeFromScope(Cpp::GetNamed("MyEnum", 0)), "1");
  Cpp::TCppScope_t TemplatedEnum_instantiated =
      Cpp::InstantiateTemplate(TemplatedEnum, &TAI_enum, 1, false);
  EXPECT_TRUE(TemplatedEnum_instantiated);

  Cpp::TCppObject_t obj = Cpp::Construct(TemplatedEnum_instantiated, nullptr, 1);
  EXPECT_TRUE(obj);
  Cpp::Destruct(obj, TemplatedEnum_instantiated, true, 0);
  obj = nullptr;

  Cpp::TCppScope_t MyNameSpace_TemplatedEnum =
      Cpp::GetScope("TemplatedEnum", Cpp::GetScope("MyNameSpace", 0));
  EXPECT_TRUE(TemplatedEnum);

  TAI_enum = Cpp::TemplateArgInfo(Cpp::GetTypeFromScope(Cpp::GetNamed(
                                      "MyEnum", Cpp::GetScope("MyNameSpace", 0))),
                                  "1");
  Cpp::TCppScope_t MyNameSpace_TemplatedEnum_instantiated =
      Cpp::InstantiateTemplate(MyNameSpace_TemplatedEnum, &TAI_enum, 1, false);
  EXPECT_TRUE(TemplatedEnum_instantiated);

  obj = Cpp::Construct(MyNameSpace_TemplatedEnum_instantiated, nullptr, 1);
  EXPECT_TRUE(obj);
  Cpp::Destruct(obj, MyNameSpace_TemplatedEnum_instantiated, true, 0);
  obj = nullptr;
  ...
  Cpp::TCppScope_t get_fn = Cpp::GetNamed("get_fn", 0);
  EXPECT_TRUE(get_fn);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_GetFunctionArgDefault) {
  ...
  Cpp::TCppScope_t my_struct =
      Cpp::InstantiateTemplate(Decls[6], template_args, 1, false);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_Construct) {
  ...
  GetAllSubDecls(Decls[1], SubDecls);
  testing::internal::CaptureStdout();
  Cpp::TCppScope_t scope = Cpp::GetNamed("C", 0);
  Cpp::TCppObject_t object = Cpp::Construct(scope, nullptr, 1);
  EXPECT_TRUE(object != nullptr);
  std::string output = testing::internal::GetCapturedStdout();
  ...
  // Placement.
  testing::internal::CaptureStdout();
  void* where = Cpp::Allocate(scope, 1);
  EXPECT_TRUE(where == Cpp::Construct(scope, where, 1));
  // Check for the value of x which should be at the start of the object.
  EXPECT_TRUE(*(int *)where == 12345);
  Cpp::Deallocate(scope, where, 1);
  output = testing::internal::GetCapturedStdout();
  EXPECT_EQ(output, "Constructor Executed");
  output.clear();
  ...
  // Pass a constructor
  testing::internal::CaptureStdout();
  where = Cpp::Allocate(scope, 1);
  EXPECT_TRUE(where == Cpp::Construct(SubDecls[3], where, 1));
  EXPECT_TRUE(*(int*)where == 12345);
  Cpp::Deallocate(scope, where, 1);
  output = testing::internal::GetCapturedStdout();
  EXPECT_EQ(output, "Constructor Executed");
  ...
  where = Cpp::Allocate(scope, 1);
  where = Cpp::Construct(Decls[2], where, 1);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_ConstructPOD) {
  ...
  EXPECT_TRUE(scope);
  object = Cpp::Construct(scope, nullptr, 1);
  EXPECT_TRUE(object);
  auto* fDouble =
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_ConstructNested) {
  ...
  testing::internal::CaptureStdout();
  Cpp::TCppScope_t scope_A = Cpp::GetNamed("A", 0);
  Cpp::TCppScope_t scope_B = Cpp::GetNamed("B", 0);
  Cpp::TCppObject_t object = Cpp::Construct(scope_B, nullptr, 1);
  EXPECT_TRUE(object != nullptr);
  std::string output = testing::internal::GetCapturedStdout();
  ...
  testing::internal::CaptureStdout();
  void* arena = Cpp::Allocate(scope_B, 1);
  EXPECT_TRUE(arena == Cpp::Construct(scope_B, arena, 1));
  ...
  Cpp::Deallocate(scope_B, arena, 1);
  output = testing::internal::GetCapturedStdout();
  EXPECT_EQ(output, "A Constructor Called\nB Constructor Called\n");
  output.clear();
}
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_Destruct) {
  ...
  Cpp::TCppScope_t scope = Cpp::GetNamed("C", 0);
  Cpp::TCppObject_t object = Cpp::Construct(scope, nullptr, 1);
  EXPECT_TRUE(Cpp::Destruct(object, scope, true, 0));
  std::string output = testing::internal::GetCapturedStdout();
  ...
  EXPECT_TRUE(Cpp::Destruct(object, scope, /*withFree=*/false, 0));
  Cpp::Deallocate(scope, object, 1);
  output = testing::internal::GetCapturedStdout();
  EXPECT_EQ(output, "Destructor Executed");
  ...
  scope = Cpp::GetNamed("D", 0);
  object = Cpp::Construct(scope, nullptr, 1);
  EXPECT_FALSE(Cpp::Destruct(object, scope, true, 0));
}
...
TYPED_TEST(CPPINTEROP_TEST_MODE, FunctionReflection_FailingTest1) {
  ...
  Cpp::TCppType_t o1 = Cpp::GetTypeFromScope(Cpp::GetNamed("o1", 0));
  Cpp::TCppType_t o2 = Cpp::GetTypeFromScope(Cpp::GetNamed("o2", 0));
  std::vector<Cpp::TCppFunction_t> fns;
  Cpp::GetClassTemplatedMethods("is_equal", Cpp::GetGlobalScope(), fns);
  EXPECT_EQ(fns.size(), 1);

  Cpp::TemplateArgInfo args[2] = {{o1}, {o2}};
  Cpp::TCppScope_t fn = Cpp::InstantiateTemplate(fns[0], args, 2, false);
  EXPECT_TRUE(fn);

  Cpp::JitCall jit_call = Cpp::MakeFunctionCallable(fn);
  EXPECT_EQ(jit_call.getKind(), Cpp::JitCall::kUnknown); // expected to fail
  EXPECT_FALSE(Cpp::Declare("int x = 1;", false));
  EXPECT_FALSE(Cpp::Declare("int y = x;", false));
}
