...
TEST(FunctionReflectionTest, InstantiateVariadicFunction) {
  ...
  std::vector<Cpp::TemplateArgInfo> args1 = {C.DoubleTy.getAsOpaquePtr(), C.IntTy.getAsOpaquePtr()};
  auto Instance1 = Cpp::InstantiateTemplate(Decls[1], args1.data(),
                                            /*type_size*/ args1.size());
  EXPECT_TRUE(Cpp::IsTemplatedFunction(Instance1));
  EXPECT_EQ(Cpp::GetFunctionSignature(Instance1), "template<> void VariadicFn<<double, int>>(double args, int args)");
  ...
  std::vector<Cpp::TemplateArgInfo> args2 = {MyClassType, C.DoubleTy.getAsOpaquePtr()};

  // instantiate VariadicFnExtended
  auto Instance2 = Cpp::InstantiateTemplate(Decls[2], args2.data(), args2.size());
  ...
  // VariadicFnExtended has one fixed param + 2 elements in TemplateArgument pack
  ...
  EXPECT_EQ(Cpp::GetFunctionSignature(Instance2), "template<> void VariadicFnExtended<<MyClass, double>>(int fixedParam, MyClass args, double args)");
}
