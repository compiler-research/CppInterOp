...
TYPED_TEST(CPPINTEROP_TEST_MODE, ScopeReflection_GetCompleteName) {
  ...
  Cpp::TCppScope_t fn = Cpp::InstantiateTemplate(Decls[11], template_args, 2, false);
  EXPECT_TRUE(fn);
...
TYPED_TEST(CPPINTEROP_TEST_MODE, ScopeReflection_InstantiateTemplateFunctionFromString) {
#if CLANG_VERSION_MAJOR == 18 && defined(CPPINTEROP_USE_CLING) &&              \
    defined(_WIN32) && (defined(_M_ARM) || defined(_M_ARM64))
  GTEST_SKIP() << "Test fails with Cling on Windows on ARM";
#endif
...
TYPED_TEST(CPPINTEROP_TEST_MODE, ScopeReflection_GetClassTemplateInstantiationArgs) {
  std::vector<Decl *> Decls;
  std::string code = R"(
...
TYPED_TEST(CPPINTEROP_TEST_MODE, ScopeReflection_GetOperator) {
  ...
  Cpp::Declare(code.c_str(), false);

  std::vector<Cpp::TCppFunction_t> ops;

  Cpp::GetOperator(Cpp::GetGlobalScope(), Cpp::Operator::OP_Plus, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 1);
  ops.clear();

  Cpp::GetOperator(Cpp::GetGlobalScope(), Cpp::Operator::OP_Minus, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 1);
  ops.clear();

  Cpp::GetOperator(Cpp::GetGlobalScope(), Cpp::Operator::OP_Star, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 0);
  ops.clear();

  // operators defined within a namespace
  Cpp::GetOperator(Cpp::GetScope("extra_ops", 0), Cpp::Operator::OP_Plus, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 2);
  ops.clear();

  // unary operator
  Cpp::GetOperator(Cpp::GetScope("extra_ops", 0), Cpp::Operator::OP_Tilde, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 1);
  ops.clear();

  Cpp::GetOperator(Cpp::GetScope("extra_ops", 0), Cpp::Operator::OP_Tilde, ops,
  ...
  Cpp::Declare(inheritance_code.c_str(), false);

  ops.clear();
  Cpp::GetOperator(Cpp::GetScope("Child", 0), Cpp::Operator::OP_Plus, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 1);

  ops.clear();
  Cpp::GetOperator(Cpp::GetScope("Child", 0), Cpp::Operator::OP_Minus, ops, Cpp::OperatorArity::kBoth);
  EXPECT_EQ(ops.size(), 1);
}
