...
TYPED_TEST(CPPINTEROP_TEST_MODE, Interpreter_Evaluate) {
  ...
  //  auto Result = Cpp::Evaluate(I, "");
  //  ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
  //  EXPECT_EQ(*Result, 0);
  // auto Result = Cpp::Evaluate(I, "__cplusplus;");
  // ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
  // EXPECT_EQ(*Result, 201402);
  // Due to a deficiency in the clang-repl implementation to get the value we
  // always must omit the ;
  TestFixture::CreateInterpreter();
  {
    auto Result = Cpp::Evaluate("__cplusplus");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 201402);
  }
  {
    auto Result = Cpp::Evaluate("#error");
  ...
  {
    auto Result = Cpp::Evaluate("int i = 11; ++i");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 12);
  }
  {
    auto Result = Cpp::Evaluate("double a = 12.; a");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 12.);
  }
  {
    auto Result = Cpp::Evaluate("float b = 13.; b");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 13.);
  }
  {
    auto Result = Cpp::Evaluate("long double c = 14.; c");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 14.);
  }
  {
    auto Result = Cpp::Evaluate("long double d = 15.; d");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 15.);
  }
  {
    auto Result = Cpp::Evaluate("unsigned long long e = 16; e");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 16);
  }
  {
    auto Result = Cpp::Evaluate("struct S{} s; s");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
  }
}
...
TYPED_TEST(CPPINTEROP_TEST_MODE, Interpreter_ActivateInterpreter) {
  ...
  EXPECT_TRUE(Cpp14 && Cpp17 && Cpp20);
  {
    auto Result = Cpp::Evaluate("__cplusplus");
    ASSERT_TRUE(bool(Result)) << llvm::toString(Result.takeError());
    EXPECT_EQ(*Result, 202002L) << "Failed to activate C++20";
  }
