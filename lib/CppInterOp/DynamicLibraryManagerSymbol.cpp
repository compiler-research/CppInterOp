...
struct LibraryPath {
  ...
  [[nodiscard]] bool hasBloomFilter() const {
...
inline mode_t cached_lstat(const char *path) {
...
StringRef GetGnuHashSection(llvm::object::ObjectFile *file) {
...
bool MayExistInElfObjectFile(llvm::object::ObjectFile *soFile,
                                    uint32_t hash) {
...
namespace Cpp {
  ...
  static std::string RPathToStr(const SmallVector<StringRef,2>& V) {
    std::string result;
    for (auto item : V)
      result += item.str() + ",";
    if (!result.empty())
  ...
  template <class ELFT>
  static void HandleDynTab(const ELFFile<ELFT>* Elf, StringRef FileName,
