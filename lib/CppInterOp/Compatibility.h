...
class DiagnosticGuard {
  ...
  bool m_oldSuppressDiags;

public:
  explicit DiagnosticGuard(clang::DiagnosticsEngine& diags, bool silent)
    : m_is_silent(silent), m_diags(diags),
      m_oldSuppressDiags(diags.getSuppressAllDiagnostics()) {
    m_diags.setSuppressAllDiagnostics(silent);
    // if (m_is_silent) {
  ...
  ~DiagnosticGuard() {
    m_diags.setSuppressAllDiagnostics(m_oldSuppressDiags);
