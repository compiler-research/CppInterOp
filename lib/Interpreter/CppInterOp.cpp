...
#define DEBUG_TYPE "jitcall"
    ...
    class FullyQualifiedTypePrinter {
    private:
      ASTContext& Context;
      PrintingPolicy Policy;

    public:
      FullyQualifiedTypePrinter(ASTContext& C)
          : Context(C), Policy(C.getPrintingPolicy()) {
        Policy.FullyQualifiedName = true;
      ...
      std::string getFullyQualifiedType(QualType QT) {
        return printTypeRecursively(QT);
      }

      std::string getFullyQualifiedTypeFromLoc(TypeLoc TL) {
        if (auto TSTL = TL.getAs<TemplateSpecializationTypeLoc>()) {
          return printTemplateSpecializationFromLoc(TSTL);
        }
        return printTypeRecursively(TL.getType());
      }

    private:
      std::string printTypeRecursively(QualType QT) {
        const clang::Type* T = QT.getTypePtr();

        switch (T->getTypeClass()) {
        case clang::Type::SubstTemplateTypeParm: {
          auto* STTP = cast<SubstTemplateTypeParmType>(T);
          return printTypeRecursively(STTP->getReplacementType());
        }
      ...
      // Generic version (no TST context)
      std::string printTemplateArgument(const TemplateArgument& Arg) {
        switch (Arg.getKind()) {
        case TemplateArgument::Type:
          return printTypeRecursively(Arg.getAsType());

        case TemplateArgument::Declaration:
          if (auto* VD = dyn_cast<ValueDecl>(Arg.getAsDecl()))
            return getFullyQualifiedName(VD);
          break;
        ...
        default: {
          ...
          Arg.print(localPolicy, OS, /*includeType=*/false);
          return result;
        }
        }
      ...
      std::string printTemplateArgument(const TemplateArgument& Arg,
        ...
        case TemplateArgument::Integral:
          return printIntegralTemplateArgument(Arg, TST);
      ...
      // 1-arg version: safe fallback
      std::string printIntegralTemplateArgument(const TemplateArgument& Arg) {
        std::string result;
        PrintingPolicy localPolicy = Policy;
        localPolicy.FullyQualifiedName = true;
        llvm::raw_string_ostream OS(result);
      ...
      // 2-arg version: richer boolean member lookup
      std::string printIntegralTemplateArgument(const TemplateArgument& Arg,
                                                const TemplateSpecializationType* TST) {
        std::string result = printIntegralTemplateArgument(Arg);

        if (Arg.getIntegralType()->isBooleanType()) {
          bool argValue = Arg.getAsIntegral().getBoolValue();
          if (argValue) {
            std::string qualified = findQualifiedBooleanMember(TST, Arg);
            if (!qualified.empty()) {
              return qualified;
            }
          }
      ...
      std::string findQualifiedBooleanMember(const TemplateSpecializationType* TST,
            ...
            if (auto* RT = TypeArg.getAsType()->getAs<RecordType>()) {
              ...
              for (auto* Member : RD->decls()) {
                if (auto* VD = dyn_cast<VarDecl>(Member)) {
                  if (VD->isStaticDataMember() && VD->getType()->isBooleanType()) {
                    if (VD->hasInit() && VD->isConstexpr()) {
                      Expr::EvalResult EvalResult;
                      if (VD->getInit()->EvaluateAsConstantExpr(EvalResult, Context)) {
                        if (EvalResult.Val.isInt()) {
                          bool memberValue = EvalResult.Val.getInt().getBoolValue();
                          bool argValue = Arg.getAsIntegral().getBoolValue();

                          if (memberValue == argValue) {
                            std::string ownerName = getFullyQualifiedName(RD);
                            return ownerName + "::" + VD->getNameAsString();
                          }
                        }
                      }
                    }
      ...
      std::string getFullyQualifiedName(const NamedDecl* ND) {
        if (!ND)
          return "";
      ...
      std::string printTemplateArgumentFromLoc(const TemplateArgumentLoc& ArgLoc) {
        const TemplateArgument& Arg = ArgLoc.getArgument();

        switch (Arg.getKind()) {
        case TemplateArgument::Type: {
          if (TypeSourceInfo* TSI = ArgLoc.getTypeSourceInfo()) {
            return getFullyQualifiedTypeFromLoc(TSI->getTypeLoc());
          }
          return printTypeRecursively(Arg.getAsType());
        }
        ...
        case TemplateArgument::Integral:
        case TemplateArgument::Expression: {
          if (Expr* E = ArgLoc.getSourceExpression()) {
      ...
      std::string printExpressionFullyQualified(const Expr* E) {
        if (!E)
          return "";

        if (auto* DRE = dyn_cast<DeclRefExpr>(E)) {
          return getFullyQualifiedName(DRE->getDecl());
        }

        if (auto* ME = dyn_cast<MemberExpr>(E)) {
          return printMemberExpression(ME);
        }

        if (auto* ICE = dyn_cast<ImplicitCastExpr>(E)) {
          return printExpressionFullyQualified(ICE->getSubExpr());
        }

        if (auto* PE = dyn_cast<ParenExpr>(E)) {
          return printExpressionFullyQualified(PE->getSubExpr());
        }

        return printComplexExpression(E);
      }

      std::string printMemberExpression(const MemberExpr* ME) {
        std::string base = printExpressionFullyQualified(ME->getBase());
        std::string member = ME->getMemberDecl()->getNameAsString();

        if (ME->isImplicitAccess() ||
            isa<CXXThisExpr>(ME->getBase()->IgnoreParenImpCasts())) {
          if (auto* VD = dyn_cast<ValueDecl>(ME->getMemberDecl())) {
            return getFullyQualifiedName(VD);
          }
        }
      ...
      std::string printComplexExpression(const Expr* E) {
        class ExpressionQualifier : public RecursiveASTVisitor<ExpressionQualifier> {
        private:
          FullyQualifiedTypePrinter& Printer;
          std::string& Result;
          bool HasQualifiedRefs = false;

        public:
          ExpressionQualifier(FullyQualifiedTypePrinter& P, std::string& R)
              : Printer(P), Result(R) {}

          bool VisitDeclRefExpr(DeclRefExpr* DRE) {
            if (!HasQualifiedRefs) {
              Result = Printer.getFullyQualifiedName(DRE->getDecl());
          ...
          bool VisitMemberExpr(MemberExpr* ME) {
            if (!HasQualifiedRefs) {
              Result = Printer.printMemberExpression(ME);
              HasQualifiedRefs = true;
            }
            return true;
          }

          bool foundQualifiedRefs() const { return HasQualifiedRefs; }
        };

        std::string result;
        ExpressionQualifier visitor(*this, result);
        visitor.TraverseStmt(const_cast<Expr*>(E));

        if (visitor.foundQualifiedRefs()) {
          return result;
        }
        ...
        PrintingPolicy localPolicy = Policy;
        localPolicy.FullyQualifiedName = true;
        E->printPretty(OS, nullptr, localPolicy);
        return OS.str();
      }
    };
    ...
    void get_type_as_string(QualType QT, std::string& type_name, ASTContext& C,
      ...
      FullyQualifiedTypePrinter printer(C);
      type_name = printer.getFullyQualifiedType(QT);
    }
...
#undef DEBUG_TYPE
    ...
    static clang::Decl* GetFunctionInstantiation(const clang::FunctionTemplateDecl * FTD, clang::QualType QT) {
      auto &S = getSema();
