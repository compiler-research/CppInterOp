...
namespace Cpp {
  ...
  TCppScope_t GetScope(TCppSema_t sema, const std::string &name,
    ...
    if (!(ND == (NamedDecl *)-1) && (llvm::isa_and_nonnull<NamespaceDecl>(ND) ||
  ...
  void get_function_params(std::stringstream &ss, FunctionDecl *FD,
    ...
    ss << "(";
    if (max_args == (size_t)-1)
      max_args = FD->param_size();
    ...
    for (auto PI = FD->param_begin(), end = FD->param_end(); PI != end; PI++) {
      ...
      if (show_formal_args) {
        ...
        if ((*PI)->hasDefaultArg()) {
          ...
          (*PI)->getDefaultArg()->printPretty(
              def_arg_os, nullptr, PrintingPolicy(Ctxt.getLangOpts()));
        }
      }
      if (++i < max_args) {
        ss << ", ";
  ...
  intptr_t GetVariableOffset(TInterp_t interp, TCppScope_t var) {
    ...
    if (auto *CXXRD = llvm::dyn_cast_or_null<CXXRecordDecl>(P)) {
      ...
      if (auto *FD = llvm::dyn_cast_or_null<FieldDecl>(D)) {
        ...
        return (intptr_t)Cxt
            .toCharUnitsFromBits(Cxt.getASTRecordLayout(CXXRD).getFieldOffset(
                FD->getFieldIndex()))
            .getQuantity();
      }
    }
