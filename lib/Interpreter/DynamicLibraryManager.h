...
private:
  ...
  /// See more information for RPATH and RUNPATH:
  /// https://en.wikipedia.org/wiki/Rpath
  ///\param[in] libStem - The filename being looked up
  ///\param[in] RPath - RPATH as provided by loader library, searching for
  /// libStem \param[in] RunPath - RUNPATH as provided by loader library,
  /// searching for libStem \param[in] libLoader - The library that loads
  /// libStem. Use "" for main program.
  ///
  ///\returns the canonical path to the file or empty string if not found
  ///
  std::string
  lookupLibInPaths(llvm::StringRef libStem,
  ...
  ///
  std::string
  lookupLibMaybeAddExt(llvm::StringRef filename,
                       llvm::SmallVector<llvm::StringRef, 2> RPath = {},
                       llvm::SmallVector<llvm::StringRef, 2> RunPath = {},
                       llvm::StringRef libLoader = "") const;
...
public:
  ...
  ///
  const SearchPathInfos& getSearchPaths() const { return m_SearchPaths; }
  ...
  /// libStem \param[in] RunPath - RUNPATH as provided by loader library,
  /// searching for libStem \param[in] libLoader - The library that loads
  /// libStem. Use "" for main program. \param[in] variateLibStem - If this
  /// param is true, and libStem is "L", then
