...
std::string GetExecutablePath() {
   ...
   return InterOp::DynamicLibraryManager::getSymbolLocation(&GetExecutablePath);
}
...
namespace InterOp {
  class Dyld {
    ...
    const InterOp::DynamicLibraryManager& m_DynamicLibraryManager;
  ...
  public:
    Dyld(const InterOp::DynamicLibraryManager &DLM,
    ...
    for (const typename ELFT::Dyn& Dyn : *DynamicEntriesOrError) {
      switch (Dyn.d_tag) {
        case ELF::DT_NEEDED:
          ...
          break;
        case ELF::DT_RPATH:
          SplitPaths(Data + Dyn.d_un.d_val, RPath, utils::kAllowNonExistant, InterOp::utils::platform::kEnvDelim, false);
          break;
        case ELF::DT_RUNPATH:
          SplitPaths(Data + Dyn.d_un.d_val, RunPath, utils::kAllowNonExistant, InterOp::utils::platform::kEnvDelim, false);
          break;
        case ELF::DT_FLAGS_1:
          ...
          if (BinObjF->isELF()) {
            ...
            for (const auto &Command : Obj->load_commands()) {
              if (Command.C.cmd == MachO::LC_LOAD_DYLIB) {
                ...
                SplitPaths(Command.Ptr + rpathCmd.path, RPath, utils::kAllowNonExistant, InterOp::utils::platform::kEnvDelim, false);
              }
            }
...
#define DEBUG_TYPE "Dyld:"
    ...
    if (!InterOp::DynamicLibraryManager::isSharedLibrary(FileName))
      return true;
