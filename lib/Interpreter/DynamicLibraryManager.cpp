...
namespace InterOp {
  ...
  using namespace InterOp::utils::platform;
  using namespace InterOp::utils;
  ...
  DynamicLibraryManager::DynamicLibraryManager()  {
    ...
    for (const char* Var : kSysLibraryEnv) {
      if (const char* Env = ::getenv(Var)) {
        ...
        SplitPaths(Env, CurPaths, utils::kPruneNonExistant, InterOp::utils::platform::kEnvDelim);
    ...
    InterOp::utils::platform::GetSystemLibraryPaths(SysPaths);
